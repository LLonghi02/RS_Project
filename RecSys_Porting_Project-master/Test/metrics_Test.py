...
class MyTestCase(unittest.TestCase):
    ...
    def test_Shannon_Entropy(self):
        ...
        shannon_entropy.recommended_counter = np.random.uniform(0, 100, n_items).astype(np.int32)
        assert  np.isclose(9.6, shannon_entropy.get_metric_value(), atol=1e-1), "metric incorrect"

        # n_items = 10000
        #
        # shannon_entropy.recommended_counter = np.random.normal(0, 50, n_items).astype(np.int32)
    ...
    def test_Diversity_list_all_equals(self):
        ...
        item_id_list = np.arange(0, n_items, dtype=np.int32)
        ...
        URM_predicted_sparse = sps.csr_matrix((URM_predicted_data, (URM_predicted_row, URM_predicted_col)), dtype=np.int32)
