...
class SearchAbstractClass(object):
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        if self.resume_from_saved and self.metadata_dict["result_on_last"] is not None and "is_last_trained_on_standard_server" in self.metadata_dict:
            self._print("{}: Resuming '{}'... Result on last already available.".format(self.ALGORITHM_NAME, self.output_file_name_root))
            return
        # if self.resume_from_saved and (os.path.exists(self.output_folder_path + self.output_file_name_root + "_best_model_last.zip") or "is_last_trained_on_standard_server" in self.metadata_dict):
        #     self._print("{}: Resuming '{}'... Result on last already available.".format(self.ALGORITHM_NAME, self.output_file_name_root))
        #     return
        ...
        # self.metadata_dict["is_last_trained_on_standard_server"] = True
