...
def plot_popularity_bias(URM_object_list, URM_name_list, output_img_path, sort_on_all = False):
    ...
    plt.xlabel('Item id')
    plt.ylabel("Number of interactions per item")
    #plt.title("Item popularity distribution for different URM splits")

    x_tick = np.arange(0,n_items, dtype=np.int32)
...
def save_popularity_statistics(URM_object_list, URM_name_list, output_file_path):
    ...
    shape = URM_object_list[0].shape
    URM_all = URM_object_list[0].astype(bool, copy=True)

    for URM_object in URM_object_list:
        assert URM_object.shape == shape
        URM_all += URM_object.astype(bool, copy=True)
    ...
    for URM_index, URM_name in enumerate(URM_name_list):
        if URM_name == "Training data":
            item_popularity_training = np.ediff1d(sps.csc_matrix(URM_object_list[URM_index]).indptr)
    ...
    for URM_index, URM_name in enumerate(URM_name_list):
        ...
        statistics_df.loc[URM_name, "Pearson Rank"] = pearsonr(item_popularity_global, item_popularity)[0]
        statistics_df.loc[URM_name, "Kendall Tau vs train"] = kendalltau(item_popularity_training, item_popularity)[0]
        statistics_df.loc[URM_name, "Pearson Rank vs train"] = pearsonr(item_popularity_training, item_popularity)[0]
        statistics_df.loc[URM_name, "Pop median"] = np.median(item_popularity)
