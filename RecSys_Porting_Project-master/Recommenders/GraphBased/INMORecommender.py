...
from Recommenders.GraphBased.normalized_adjacency_matrix import normalized_adjacency_matrix, from_sparse_to_tensor
...
def graph_rank_nodes(n_users, n_items, adj_mat, ranking_metric):
    ...
    if ranking_metric == 'degree':
        ...
        pr = nx.pagerank(g)
        pr = np.array([pr[i] if i in pr else 0 for i in range(n_users + n_items)])
...
class INMORecommender(BaseMatrixFactorizationRecommender, Incremental_Training_Early_Stopping):
    """
    """

    RECOMMENDER_NAME = "INMORecommender"

    def __init__(self, URM_train, use_gpu = False, use_cython_sampler = True, verbose = True):
        super(INMORecommender, self).__init__(URM_train, verbose = verbose)
