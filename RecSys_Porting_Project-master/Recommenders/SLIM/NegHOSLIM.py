...
#     MM = sps.lil_matrix((len(ii_pairs[0]), X.shape[1]),    dtype=np.float32)
...
class NegHOSLIMRecommender_dense(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, epochs=300, feature_pairs_threshold = None, feature_pairs_n = 100, lambdaBB = 5e2, lambdaCC = 5e3, rho = 1e5,
        ...
        # initialize
        self.CC = np.zeros( (ZtZ.shape[0], self.XtX.shape[0]),dtype=np.float32 )
        self.DD = np.zeros( (ZtZ.shape[0], self.XtX.shape[0]),dtype=np.float32 )
        self.UU = np.zeros( (ZtZ.shape[0], self.XtX.shape[0]),dtype=np.float32 ) # is Gamma in paper
        self.BB = np.zeros( (self.URM_train.shape[0], self.URM_train.shape[0]),dtype=np.float32 )
...
class NegHOSLIMRecommender(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, epochs=300, feature_pairs_threshold = None, feature_pairs_n = 100, lambdaBB = 5e2, lambdaCC = 5e3, rho = 1e5,
        ...
        # initialize
        self.CC = np.zeros((self.n_feature_pairs, self.n_items), dtype=np.float32 )
        self.DD = np.zeros((self.n_feature_pairs, self.n_items), dtype=np.float32 )
        self.UU = np.zeros((self.n_feature_pairs, self.n_items), dtype=np.float32 ) # is Gamma in paper
        self.BB = np.zeros((self.n_items, self.n_items), dtype=np.float32 )
    ...
    def fit(self, l1_ratio=0.1, alpha = 1.0, positive_only_weights = False, topK = 100, feature_pairs_threshold = None, feature_pairs_n = 1000):
        ...
        for current_item in range(self.n_items):
            ...
            similarity_builder.add_data_lists(row_list_to_add=nonzero_model_coef_index,
                                              col_list_to_add=np.ones(len(nonzero_model_coef_index), dtype = np.int32) * current_item,
